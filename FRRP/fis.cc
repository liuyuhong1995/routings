...
static DOUBLE defuzzSmallestOfMax(FIS *fis, int m, DOUBLE *mf, int numofpoints)
{
	...
	DOUBLE mf_max;
	int i, min_index = 0;
	DOUBLE min_distance = MAX(ABS(max), ABS(min)); /* initialized with the boundary having maximum magnitude */
...
static DOUBLE defuzzLargestOfMax(FIS *fis, int m, DOUBLE *mf, int numofpoints)
{
	...
	DOUBLE mf_max;
	int i, max_index = 0;
	DOUBLE max_distance = -1.0; /* initialized with a negative value since a magnitude calculated in the loop is always positive. */
	DOUBLE distance; /* distance to the origin */
...
void fisEvaluate(FIS *fis, int numofpoints)
{
	...
	for (i = 0; i < fis->out_n; i++) {
		...
		if (strcmp(fis->defuzzMethod, "wtaver") == 0)
            ...
            sprintf(errorMessage, "Specified defuzzification method \"%s\" is not valid for a \"sugeno\" type FIS.", fis->defuzzMethod);
            // fisError(errorMessage);
		}
	}
	}
	...
	/*	double aggMF[MF_POINT_N];
		...
		if (!fis->userDefinedDefuzz){
            if ((strcmp(fis->defuzzMethod, "wtaver") == 0) || (strcmp(fis->defuzzMethod, "wtsum") == 0)){
                char errorMessage[100];
				
                sprintf(errorMessage, "Specified defuzzification method \"%s\" is not valid for a \"mamdani\" type FIS.", fis->defuzzMethod);
                // fisError(errorMessage);
            }
...
static void
#ifdef __STDC__
getMfN(const char *filename, int in_n, DOUBLE *in_mf_n, int out_n, DOUBLE *out_mf_n)
#else
...
static DOUBLE **
#ifdef __STDC__
returnEmptyFismatrix(const char *filename, int *row_n_p, int *col_n_p)
#else
...
DOUBLE **
#ifdef __STDC__
returnFismatrix(const char *fis_file, int *row_n_p, int *col_n_p)
#else
...
DOUBLE **returnDataVector(DOUBLE *a, DOUBLE *b, DOUBLE *c, DOUBLE *d) {
	DOUBLE **datamatrix;
	datamatrix = (DOUBLE **)fisCreateMatrix(1, 4, sizeof(DOUBLE));
	datamatrix[0][0] = (*a)/1000000000;
	datamatrix[0][1] = *b;
	datamatrix[0][2] = *c;
	datamatrix[0][3] = *d;
	return(datamatrix);
