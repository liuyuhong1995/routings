...
void InstallEnergy(NodeContainer senseNodes,NodeContainer relayNodes){
	...
	for (uint32_t i = 0; i < relayNodes.GetN(); i++) {
		remaingJ[relayNodes.Get(i)->GetId()] = initialJ;
	}
...
void EnergyFinalRecord(NodeContainer senseNodes,double remaingJ[]){
		std::stringstream ss;
		std::stringstream si;
		ss << "/home/wsn/sim_temp/GROUPROCOMAR/"<<nRelayNodes<<"relay-final.record";
...
void UpdateEnergySources(Ptr<Node> n, Ptr<Packet> p, uint16_t flag,NodeContainer senseNodes,NodeContainer relayNodes,NodeContainer mobileSinkNode) {
	...
	uint32_t thisPktSize = p->GetSize();
	 nodeType thisNt = CheckNodeType(n,senseNodes,relayNodes,mobileSinkNode.Get(0));
