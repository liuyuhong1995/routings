...
enum nodeType CheckNodeType(Ptr<Node> node,NodeContainer senseNodes,NodeContainer sinkNodes,Ptr<Node> mobileNode) {
	if(node->GetId()==mobileNode->GetId())
		return mobileSink_Type;
...
Ptr<Node> GetNodePtrFromIpv4Adr(Ipv4Address adr,NodeContainer senseNodes,NodeContainer sinkNodes,NodeContainer mobileSinkNode) {
	// for (NodeContainer::Iterator i = sinkNodes.Begin(); i != sinkNodes.End();
	// 		i++) {
	// 	Ptr<Node> n = *i;
	// 	if (GetNodeIpv4Address(n) == adr) {
	...
	for (NodeContainer::Iterator i = senseNodes.Begin(); i != senseNodes.End();
		...
		if (GetNodeIpv4Address(n) == adr) {
			NS_LOG_LOGIC("GetNodePtrFromIpv4Adr: senseNodes matched!");
			// cout<<"通过ip地址找到�?,几点id�?"<<n->GetId()<<endl;
			return n;
		}
	}
	...
	for (NodeContainer::Iterator i = sinkNodes.Begin(); i != sinkNodes.End();
		...
		Ptr<Node> n = *i;
		if (GetNodeIpv4Address(n) == adr) {
			NS_LOG_LOGIC("GetNodePtrFromIpv4Adr: sinkNodes matched!");
			// cout<<"通过ip地址找到�?,几点id�?"<<n->GetId()<<endl;
			return n;
		}
	}
