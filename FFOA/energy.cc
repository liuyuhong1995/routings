...
void InstallEnergy(NodeContainer senseNodes,NodeContainer sinkNodes){
	...
	for (uint32_t i = 0; i < senseNodes.GetN(); i++) {
     ...
     for (uint32_t i = 0; i < sinkNodes.GetN(); i++) {
		remaingJ[sinkNodes.Get(i)->GetId()] = initialJ*10;
	}
}
/*
...
void EnergyFinalRecord(NodeContainer senseNodes,double remaingJ[]){
		std::stringstream ss;
		std::stringstream si;
		ss << "/home/wsn/jgy/sim_temp/"<<nSinkNodes<<"relay-final.record";
		...
		// ss << std::endl << "开始时�?:" << simStartRealTime << std::endl;
		// ss << std::endl << "结束时间: " << simFinishRealTime << std::endl;
		// ss << std::endl << "发送数�?�?:" << totalBytesGenerated << std::endl;
		// ss << std::endl << "接收数据�?:" << totalBytesGathered << std::endl;
		// ss << std::endl << "系统吞吐�?: " << totalBytesGathered*1.0/((simFinishRealTime-simStartRealTime)/1000)<<"(Byte/s)" << std::endl;
...
void DoDrainRecord(Ptr<Node> thisNode){
	...
	// Vector thisLocation = cpmm->GetPosition();	//获取当前节点的坐�?
